...
class PatientGenerator:
    ...
    def __init__(self, data_path='preprocessed_nhanes/nhanes_final_complete.csv', device=None):
        ...
        self.model_path = Path('models/generator/patient_generator_gan.pt')
        if self.model_path.exists():
            try:
                checkpoint = torch.load(self.model_path, map_location=self.device, weights_only=False)
                self.generator.load_state_dict(checkpoint['generator'])
                self.discriminator.load_state_dict(checkpoint['discriminator'])
                print(f"[OK] Loaded pretrained model from {self.model_path}")
                print(f"  Training epochs: {checkpoint.get('epoch', 'unknown')}")
            except Exception as e:
                print(f"[WARNING] Failed to load pretrained model: {e}")
                print("  Will use random initialization - train the model first!")
        else:
            print(f"[INFO] No pretrained model found at {self.model_path}")
    ...
    def train(self, epochs=200, batch_size=128, lr=0.0002, 
              save_every=50, show_progress_every=10):
        """
